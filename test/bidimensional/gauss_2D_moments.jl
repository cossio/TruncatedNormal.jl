...
@testset "gauss2Dtruncstats" begin
    ...
    tμ, tΣ = gauss2Dtruncstats(μ, Σ, a, b)
    # the following values were obtained using matlab/sample_bivariate_trunc.m
    @test tμ[1] ≈ -0.205806453856253    rtol=1e-4
    @test tμ[2] ≈ 0.699184016211862     rtol=1e-4
    @test tΣ[1,1] ≈ 0.042415563153775   rtol=1e-4
    @test tΣ[2,2] ≈ 0.488875575709699   rtol=1e-4
    @test tΣ[1,2] ≈ -0.143896434017694  rtol=1e-4
end