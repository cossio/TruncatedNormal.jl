...
using TruncatedNormal: _F1, _F2

f1(x,y) = (exp(-y^2) - exp(-x^2)) / (erf(y) - erf(x))
f2(x,y) = (exp(-y^2)y - exp(-x^2)x) / (erf(y) - erf(x))
...
@testset "F1 & F2" begin
    ...
    @testset "Exhaustive" begin
        ...
        for x = (-v, v), y = (-v, v)
            x == y && continue
            @test _F1(Float64(x), Float64(y)) ≈ f1(x, y)
            @test _F2(Float64(x), Float64(y)) ≈ f2(x, y)
        end
